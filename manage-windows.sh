#!/usr/bin/env bash
# ============================================================================
#  Al Islam al-Muftara Alayh Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/07-al-islam-al-muftara-alayh}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.alislamalmuftaraalayh}"
AVD_NAME="${AVD_NAME:-al_islam_al_muftara_alayh_test}"
...
cmd_test() {
  ...
  local out="/tmp/al-islam-al-muftara-alayh-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Al Islam al-Muftara Alayh Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
